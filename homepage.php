<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>FOODIE - A Food Ordering Web App</title>
  <link href="homepage.css" rel="stylesheet">
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0-beta1/dist/css/bootstrap.min.css" rel="stylesheet"
    integrity="sha384-0evHe/X+R7YkIZDRvuzKMRqM+OrBnVFBL6DOitfPri4tjfHxaWutUpFmBp4vmVor" crossorigin="anonymous">
  <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.8.3/font/bootstrap-icons.css">
</head>

<body>
  <?php 
        session_start();
        if(isset($_POST["submit"])){
            unset($_SESSION["success"]);
        }
    ?>
  <?php if(isset($_SESSION["success"])) { ?>
  <div align="center" class="alert alert-success" role="alert">
    <?php echo $_SESSION["success"]; ?>
  </div>
  <?php } ?>
  <div class="hero">
    <nav class="navbar navbar-expand-lg">
      <div class="container-fluid">
        <a class="navbar-brand" href="#"><span
            style="font-size: 35px;color: red; font-weight: bolder;">FOODIE</span></a>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav"
          aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
          <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="navbarNav">
          <ul class="last navbar-nav">
            <li class="nav-item">
              <a id="link" class="nav-link active" aria-current="page" href="#">Home</a>
            </li>
            <li class="nav-item">
              <a id="link" class="nav-link active" href="#contactus">Contact us</a>
            </li>
            <li class="nav-item">
              <a id="link" class="nav-link active" href="myorders.php">My orders</a>
            </li>
            <li class="nav-item">
              <a id="link" class="nav-link active" href="cart.php">My cart</a>
            </li>
          </ul>
        </div>
      </div>
    </nav>
    <?php if(isset($_SESSION["success"])){ ?>
    <div class="account btn-group">
      <button type="button" class="btn btn-primary dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
        <?php echo $_SESSION["mail"]; ?>
      </button>
      <ul class="dropdown-menu">
        <form action="<?php htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
          <input class="dropdown-item" type="submit" name="submit" value="Log out">
        </form>
      </ul>
    </div>
    <?php } ?>
    <?php if(!isset($_SESSION["success"])){ ?>
      <div class="account">
      <a class="btn btn-primary" href="login.php">LOG IN</a>
      <a class="btn btn-primary" href="signup.php">SIGN UP</a>
      </div>
    <?php } ?>
    <div align="center">
      <div style="margin-top: 100px;" class="row">
        <h1 style="color: darkorange;">FEELING HUNGRY?</h1>
        <h2 style="color: white;">Order and try your favorite dish now!</h2>
      </div>
      <div style="margin-top: 100px;" class="row">
        <form class="hero_form" action="searchresults.php" method="POST">
          <input type="text" name="key" placeholder="Search a variety of dishes here..." required>
          <button type="submit" name="search" value="dishes"><i class="bi bi-chevron-right"></i></button>
        </form>
      </div>
    </div>
  </div><br><br>
  <h2 align="center" style="text-decoration: underline;color: darkgreen;">SEE CATEGORIES:</h2>
  <div class="categories">
    <div class="card" style="width: 18rem;">
      <img class="card-img-top" src="veg.jpg" alt="Card image cap">
      <div class="card-body">
        <h5 class="card-title">VEG FOODS</h5>
        <p class="card-text">Explore a different varieties of vegetarian foods you love by clicking below.</p>
        <a href="vegfoods.php" class="btn btn-primary">See dishes</a>
      </div>
    </div>
    <div class="card" style="width: 18rem;">
      <img class="card-img-top" src="non-veg.jpg" alt="Card image cap">
      <div class="card-body">
        <h5 class="card-title">NON-VEG FOODS</h5>
        <p class="card-text">Explore a different varieties of non-vegetarian foods you love by clicking below.</p>
        <a href="non-vegfoods.php" class="btn btn-primary">See dishes</a>
      </div>
    </div>
    <div class="card" style="width: 18rem;">
      <img class="card-img-top" src="cool_drinks.jpg" alt="Card image cap">
      <div class="card-body">
        <h5 class="card-title">COOL DRINKS AND BEVERAGES</h5>
        <p class="card-text">Feeling dehydrated? Cool yourself by exploring a different cool drinks by clicking below.
        </p>
        <a href="cooldrinks.php" class="btn btn-primary">See dishes</a>
      </div>
    </div>
  </div><br><br>
  <div class="container">
    <h2 align="center" style="text-decoration: underline;color: darkgreen;">WHAT WE HAVE:</h2>
    <div class="row justify-content-center">
      <div class="col-md-2 text-center">
        <script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script>
        <lottie-player src="https://assets6.lottiefiles.com/packages/lf20_w9exmcol.json" background="transparent"
          speed="1" style="width: 250px; height: 250px;" autoplay></lottie-player>
      </div>
      <div class="col-md-6 text-center">
        <h3 style="margin-top: 100px;">24/7 Customer Care support</h3>
      </div>
    </div>
    <hr>
    <div class="row justify-content-center">
      <div class="col-md-2 text-center">
        <script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script>
        <lottie-player src="https://assets2.lottiefiles.com/packages/lf20_6sxyjyjj.json" background="transparent"
          speed="1" style="width: 300px; height: 300px;" autoplay></lottie-player>
      </div>
      <div class="col-md-6 text-center">
        <h3 style="margin-top: 100px;">On-Time Delivery</h3>
      </div>
    </div>
    <hr>
    <div class="row justify-content-center">
      <div class="col-md-2 text-center">
        <script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script>
        <lottie-player src="https://assets2.lottiefiles.com/packages/lf20_vivylat3.json" background="transparent"
          speed="1" style="width: 300px; height: 300px;" autoplay></lottie-player>
      </div>
      <div class="col-md-6 text-center">
        <h3 style="margin-top: 100px;">Affordable price with best quality</h3>
      </div>
    </div>
    <hr>
  </div>
  <div class="container" id="contactus">
    <h2 align="center" style="text-decoration: underline;color: darkgreen;">CONTACT US:</h2><br><br>
    <div>
      <form action="https://formsubmit.co/cff94b086610b6f090d145101db52e39" method="POST">
        YOUR NAME:
        <input type="text" class="form-control" name="name" placeholder="Name" required><br>
        YOUR EMAIL ID:
        <input type="email" class="form-control" name="email" placeholder="Email id" required><br>
        YOUR MESSAGE:
        <textarea cols="30" rows="10" placeholder="Feedback or queries..." name="message" class="form-control"
          required></textarea><br>
        <button type="submit" class="btn btn-info send">SEND MESSAGE</button>
      </form><br><br>
    </div>
  </div><br><br>
  <footer>
    <div class="row">
      <div class="col-md-3">
        <h3>Your account:</h3>
        <h5><a href="login.php">Login</a></h5>
        <h5><a href="signup.php">Register</a></h5>
      </div>
      <div class="col-md-3">
        <h3>Categories:</h3>
        <h5><a href="vegfoods.php">Veg foods</a></h5>
        <h5><a href="non-vegfoods.php">Non-veg foods</a></h5>
        <h5><a href="cooldrinks.php">Cool drinks</a></h5>
      </div>
      <div class="col-md-3">
        <h3>Contact details:</h3>
        <h5 style="color: white;">EMAIL ID:</h5>
        <p style="color: white;">kumaresharul2003@gmail.com</p>
        <h5 style="color: white;">CONTACT NO:</h5>
        <p style="color: white;">8903507021</p>
      </div>
      <div class="col-md-3 social text-center">
        <h3>Social media links:</h3>
        <a href="https://www.facebook.com/kumaresh.a.921" target="_blank" class="col-2"><i
            style="font-size: 35px;color: darkblue;" class="bi bi-facebook"></i></a>
        <a href="https://www.instagram.com/kumaresharul/" target="_blank" class="col-2"><i
            style="font-size: 35px;color: darkorchid;" class="bi bi-instagram"></i></a>
        <a href="https://twitter.com/Kumaresharul007" target="_blank" class="col-2"><i
            style="font-size: 35px;color: skyblue;" class="bi bi-twitter"></i></a>
        <a href="https://www.linkedin.com/in/kumaresh-arul-62854321b/" target="_blank" class="col-2"><i
            style="font-size: 35px;color: darkblue;" class="bi bi-linkedin"></i></a>
      </div>
    </div><br>
    <hr><br>
    <h5 align="center">created by <b style="color: white;">Kumaresh Arul</b> | &copy;2022</h5>
  </footer>
  <!-- JavaScript Bundle with Popper -->
  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0-beta1/dist/js/bootstrap.bundle.min.js"
    integrity="sha384-pprn3073KE6tl6bjs2QrFaJGz5/SUsLqktiwsUTF55Jfv3qYSDhgCecCxMW52nD2"
    crossorigin="anonymous"></script>
</body>

</html>